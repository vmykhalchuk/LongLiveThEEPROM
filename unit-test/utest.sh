...
# Compile
arduino-cli compile -v --fqbn arduino:avr:nano --output-dir $TARGET_DIR $INO_BUILD_DIR
if [ $? -ne 0 ]; then
...
# Run simulation
./call_simavr.sh $TARGET_DIR/$INO_FILE_NAME.elf $BUILD_DIR/serial.out "@!@TEST_COMPLETE@!@" "@!@TEST_FAILED@!@"
if [ $? -ne 0 ]; then
